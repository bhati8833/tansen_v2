...
url = f"https://{env['CPANEL_HOST']}:{env['CPANEL_PORT']}/execute/Fileman/get_file_content?dir=public_html&file=index.html"
req = urllib.request.Request(url, headers={"Authorization": f"cpanel {env['CPANEL_USER']}:{env['CPANEL_TOKEN']}"})
try:
    resp = urllib.request.urlopen(req, context=ctx)
    res = json.loads(resp.read().decode())
    print("Content of public_html/index.html:")
    print(res['data']['content'][:500])
